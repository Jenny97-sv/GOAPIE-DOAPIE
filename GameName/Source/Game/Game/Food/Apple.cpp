...
bool Apple::Init()
{
	...
	tastiness = Random::GetRandomNumber<float>(4.f, 6.f);
	healthiness = Random::GetRandomNumber<float>(7.f, 10.f);
...
bool Apple::InitAnimations()
{
	...
	if (!s0->Init("Sprites/Apple1.dds")) return false;

	std::unique_ptr<Sprite> s1 = std::make_unique<Sprite>();
	if (!s1->Init("Sprites/Apple2.dds")) return false;

	std::unique_ptr<Sprite> s2 = std::make_unique<Sprite>();
	if (!s2->Init("Sprites/Apple3.dds")) return false;
	...
	if (!t0->Init("Sprites/AppleHalf1.dds")) return false;

	std::unique_ptr<Sprite> t1 = std::make_unique<Sprite>();
	if (!t1->Init("Sprites/AppleHalf2.dds")) return false;

	std::unique_ptr<Sprite> t2 = std::make_unique<Sprite>();
	if (!t2->Init("Sprites/AppleHalf3.dds")) return false;
