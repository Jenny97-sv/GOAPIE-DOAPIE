...
namespace CommonUtilities
{
	...
	public:
		...
		{
			Matrix4x4<T> result;
			result(4, 1) = aScaleVector.x;
			result(4, 2) = aScaleVector.y;
			result(4, 3) = aScaleVector.z;
	...
	template <class T> T& Matrix4x4<T>::operator()(const int aRow, const int aColumn)
	{
		return objects[aRow - 1][aColumn - 1];
	}
	template <class T> const T& Matrix4x4<T>::operator()(const int aRow, const int aColumn) const
	{
		return objects[aRow - 1][aColumn - 1];
	}
	template<class T>
	inline void Matrix4x4<T>::SetPosition(const CommonUtilities::Vector3<T>& aVector3)
	{
		objects[3][0] = aVector3.x;
		objects[3][1] = aVector3.y;
		objects[3][2] = aVector3.z;
	}


	template <class T> void Matrix4x4<T>::operator=(const Matrix4x4<T>& aMatrix)
	{
	...
	template <class T> Matrix4x4<T> operator*(const Matrix4x4<T>& aMatrix0, const Matrix4x4<T>& aMatrix1)
	{
		...
		for (int i = 1; i <= 4; i++)
		{
			for (int j = 1; j <= 4; j++)
			{
				tempMatrix(i, j) = aMatrix0(i, 1) * aMatrix1(1, j) + aMatrix0(i, 2) * aMatrix1(2, j) + aMatrix0(i, 3) * aMatrix1(3, j) + aMatrix0(i, 4) * aMatrix1(4, j);
			}
		}
		return tempMatrix;
	}
	template <class T> Vector4<T> operator*(const Vector4<T>& aVector, const Matrix4x4<T>& aMatrix)
	{
		...
		tempVector.x = aVector.x * aMatrix(1, 1) + aVector.y * aMatrix(2, 1) + aVector.z * aMatrix(3, 1) + aVector.w * aMatrix(4, 1);
		tempVector.y = aVector.x * aMatrix(1, 2) + aVector.y * aMatrix(2, 2) + aVector.z * aMatrix(3, 2) + aVector.w * aMatrix(4, 2);
		tempVector.z = aVector.x * aMatrix(1, 3) + aVector.y * aMatrix(2, 3) + aVector.z * aMatrix(3, 3) + aVector.w * aMatrix(4, 3);
		tempVector.w = aVector.x * aMatrix(1, 4) + aVector.y * aMatrix(2, 4) + aVector.z * aMatrix(3, 4) + aVector.w * aMatrix(4, 4);
	...
	template <class T> void operator+=(Matrix4x4<T>& aMatrix0, const Matrix4x4<T>& aMatrix1)
	{
		for (int row = 1; row <= 4; row++)
		{
			for (int column = 1; column <= 4; column++)
	...
	template<class T> inline Matrix4x4<T> Matrix4x4<T>::CreateRotationAroundX(T aAngleInRadians)
	{
		Matrix4x4<T> temp;
	...
	template<class T> inline Matrix4x4<T> Matrix4x4<T>::CreateRotationAroundY(T aAngleInRadians)
	{
		Matrix4x4<T> temp;
	...
	template<class T> inline Matrix4x4<T> Matrix4x4<T>::CreateRotationAroundZ(T aAngleInRadians)
	{
		Matrix4x4<T> temp;
		temp(1, 1) = std::cos(aAngleInRadians);
	...
	template<class T> inline Matrix4x4<T> Matrix4x4<T>::GetFastInverse(const Matrix4x4<T>& aTransform)
	{
		...
		newMatrix(4, 1) = -aTransform(4, 1) * newMatrix(1, 1) - aTransform(4, 2) * newMatrix(2, 1) - aTransform(4, 3) * newMatrix(3, 1);
		newMatrix(4, 2) = -aTransform(4, 1) * newMatrix(1, 2) - aTransform(4, 2) * newMatrix(2, 2) - aTransform(4, 3) * newMatrix(3, 2);
		newMatrix(4, 3) = -aTransform(4, 1) * newMatrix(1, 3) - aTransform(4, 2) * newMatrix(2, 3) - aTransform(4, 3) * newMatrix(3, 3);
