...
#pragma region IMPLICIT CONSTRUCTION, ASSIGNMENT, ETC... OF OTHER TYPE

		template <class U>
		Vector2<T>(const U& aX, const U& aY) : x(static_cast<T>(aX)), y(static_cast<T>(aY)) {}

		template <class U>
		explicit operator Vector2<U>() { return { static_cast<U>(x), static_cast<U>(y) }; }

		template <class U>
		Vector2<T>& operator=(const Vector2<U>& aVector) { x = static_cast<T>(aVector.x); y = static_cast<T>(aVector.y); return *this; }
...
#pragma endregion
		...
		static Vector2<T> Lerp(Vector2<T> aStart, Vector2<T> anEnd, float t);
	};
	...
	template <class T> bool operator==(const Vector2<T>& vector1, const Vector2<T>& vector2)
	{
	...
	template<class T>
	Vector2<T> Vector2<T>::Lerp(Vector2<T> aStart, Vector2<T> anEnd, float t)
	{
